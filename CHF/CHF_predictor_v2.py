...
def main():
    ...
    return (Q180, Q360, Q450, Q720, ufm, flags)
...
def queries (dx = diagnoses_doc, adm = admissions_doc):
    ...
    for s in subj:
        ...
        #compare t_init with t_final for each subject
        difference = t[-1][0] - t[0][0]
        
        if (difference.days) <=180: 
            if (not df[(df['HADM_ID']==t[0][1]) & (df['ICD9_CODE'].isin(CHF))].empty) & (not df[(df['HADM_ID']==t[-1][1]) & (df['ICD9_CODE'].isin(CHF))].empty): 
                y_pos180.append([s, t[0][1], t[-1][1]])        
            else:
                y_neg180.append([s, t[0][1], t[-1][1]])
                
        elif (180<difference.days <=360): 
            if (not df[(df['HADM_ID']==t[0][1]) & (df['ICD9_CODE'].isin(CHF))].empty) & (not df[(df['HADM_ID']==t[-1][1]) & (df['ICD9_CODE'].isin(CHF))].empty): 
                y_pos360.append([s, t[0][1], t[-1][1]])        
            else:
                y_neg360.append([s, t[0][1], t[-1][1]])
                
        elif (360<difference.days <=450): 
            if (not df[(df['HADM_ID']==t[0][1]) & (df['ICD9_CODE'].isin(CHF))].empty) & (not df[(df['HADM_ID']==t[-1][1]) & (df['ICD9_CODE'].isin(CHF))].empty): 
                y_pos450.append([s, t[0][1], t[-1][1]])        
            else:
                y_neg450.append([s, t[0][1], t[-1][1]])
                
        elif (450<difference.days <=720): 
            if (not df[(df['HADM_ID']==t[0][1]) & (df['ICD9_CODE'].isin(CHF))].empty) & (not df[(df['HADM_ID']==t[-1][1]) & (df['ICD9_CODE'].isin(CHF))].empty): 
                y_pos720.append([s, t[0][1], t[-1][1]])        
            else:
                y_neg720.append([s, t[0][1], t[-1][1]])     
...
if __name__ == '__main__':
    ...
    Q180, Q360, Q450, Q720, ufm, flags= main()
